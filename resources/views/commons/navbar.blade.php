<header class="mb-4">
    <nav class="navbar bg-white text-neutral-content">
        {{-- トップページへのリンク --}}
        <div class="flex-1">
            <h1><a class="btn btn-ghost normal-case text-xl text-primary fs-3" href="{{ route('dashboard') }}">Xposts</a></h1>
        </div>
        ...
        <div class="flex-none">
            <form method="POST" action="{{ route('logout') }}">
                ...
                <!--<div class="dropdown dropdown-end">
                    <button type="button" tabindex="0" class="btn normal-case font-normal lg:hidden link-underline-opacity-0">
                        @if (Auth::check())
                            {{ Auth::user()->name }}
                        @else
                            Guest
